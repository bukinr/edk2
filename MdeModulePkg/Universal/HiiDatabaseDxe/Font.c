  ...
  while (*BlockPtr != EFI_HII_GIBT_END) {
    switch (*BlockPtr) {
      ...
      case EFI_HII_GIBT_EXT1:
        BlockPtr += *(UINT8 *)((UINTPTR_T)BlockPtr + sizeof (EFI_HII_GLYPH_BLOCK) + sizeof (UINT8));
        break;
      case EFI_HII_GIBT_EXT2:
        CopyMem (
          &Length16,
          (UINT8 *)((UINTPTR_T)BlockPtr + sizeof (EFI_HII_GLYPH_BLOCK) + sizeof (UINT8)),
          sizeof (UINT16)
          );
        BlockPtr += Length16;
        break;
      ...
      case EFI_HII_GIBT_GLYPH:
        ...
        if (CharCurrent == CharValue) {
          return WriteOutputParam (
                   (UINT8 *)((UINTPTR_T)BlockPtr + sizeof (EFI_HII_GIBT_GLYPH_BLOCK) - sizeof (UINT8)),
                   BufferLen,
                   &LocalCell,
                   GlyphBuffer,
                   Cell,
